...
const allCategories = [...new Set(topWeapons.map(w => w.category))];
const allCountries = [...new Set(topWeapons.map(w => w.country))];
...
const Explore = () => {
  ...
  const [sortBy, setSortBy] = useState<"power" | "votes" | "range" | "speed">("power");
  ...
  const filtered = useMemo(() => {
    return topWeapons
      .filter(w => {
        ...
        if (w.range < rangeMin) return false;
        if (w.speed < speedMin) return false;
        return true;
      })
      .sort((a, b) => {
  ...
  }, [search, category, country, rangeMin, speedMin, sortBy, getVotes]);
      ...
      <div className="flex gap-3 px-4 py-4">
        ...
        {showFilters && (
          <div className="w-[220px] flex-shrink-0 hidden lg:flex flex-col gap-4 glass-panel p-4 h-fit sticky top-[60px]">
            ...
            <div>
              ...
              <input type="range" min={0} max={18000} step={500} value={rangeMin}
        ...
        <div className="flex-1 min-w-0 space-y-3">
          ...
          <div className="glass-panel p-3 flex items-center gap-3 flex-wrap">
            <button onClick={() => setShowFilters(!showFilters)} className="p-2 rounded-md hover:bg-muted/50 transition-colors lg:hidden">
          ...
          {view === "grid" ? (
              ...
              {filtered.map((w, i) => (
                <div key={w.name} onClick={() => navigate(`/weapon/${encodeURIComponent(w.name)}`)}
                  ...
                  <div className="flex items-start justify-between mb-2">
                    <span className={`text-2xl font-bold ${w.rank <= 3 ? ["text-neon-gold", "text-neon-silver", "text-neon-bronze"][w.rank - 1] : "text-muted-foreground"}`}>
                      #{w.rank}
                    </span>
                    <span className="text-[10px] px-2 py-0.5 rounded bg-neon-cyan/10 text-neon-cyan border border-neon-cyan/20">{w.category}</span>
                  </div>
                  <h3 className="text-base font-bold text-foreground">{w.name}</h3>
                  <div className="flex items-center gap-1.5 mt-1 text-sm text-muted-foreground">
                    <span>{w.countryCode}</span> {w.country}
                  </div>
                  <div className="grid grid-cols-3 gap-2 mt-3 text-[10px]">
                    <div><span className="text-muted-foreground block">Range</span><span className="font-mono-tech text-foreground">{w.range.toLocaleString()} km</span></div>
                    <div><span className="text-muted-foreground block">Speed</span><span className="font-mono-tech text-foreground">Mach {w.speed}</span></div>
                    <div><span className="text-muted-foreground block">Power</span><span className="font-mono-tech text-foreground">{w.power}/100</span></div>
                  </div>
                  <div className="w-full h-1.5 rounded-full bg-muted overflow-hidden mt-3">
                    <div className={`h-full rounded-full ${powerBarColor(w.power)} transition-all duration-700`} style={{ width: `${w.power}%` }} />
                  </div>
            ...
            <div className="glass-panel overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filtered.map((w, i) => (
                    <tr key={w.name} onClick={() => navigate(`/weapon/${encodeURIComponent(w.name)}`)}
                      className="border-b border-border/20 hover:bg-muted/20 cursor-pointer transition-colors animate-row-in"
                      style={{ animationDelay: `${i * 50}ms` }}>
                      <td className="px-4 py-2.5 font-bold text-muted-foreground">{w.rank}</td>
                      <td className="px-4 py-2.5 font-semibold text-foreground">{w.name}</td>
                      <td className="px-4 py-2.5"><span className="mr-1">{w.countryCode}</span>{w.country}</td>
                      <td className="px-4 py-2.5 text-muted-foreground">{w.category}</td>
                      <td className="px-4 py-2.5 text-right font-mono-tech">{w.range.toLocaleString()} km</td>
                      <td className="px-4 py-2.5 text-right font-mono-tech">Mach {w.speed}</td>
                      <td className="px-4 py-2.5 text-right font-mono-tech">{w.power}</td>
                      <td className="px-4 py-2.5 text-right font-mono-tech text-neon-cyan">{getVotes(w.name).toLocaleString()}</td>

...
import { ArrowRightLeft, ChevronUp, LayoutGrid, List } from "lucide-react";
...
const Leaderboard = () => {
  ...
  const [voteCounts, setVoteCounts] = useState<Record<number, number>>(
    Object.fromEntries(topWeapons.map(w => [w.rank, w.votes]))
  );
  const [votedItems, setVotedItems] = useState<Set<number>>(new Set());

  const handleVote = (rank: number) => {
    if (votedItems.has(rank)) return;
    setVoteCounts(prev => ({ ...prev, [rank]: prev[rank] + 1 }));
    setVotedItems(prev => new Set(prev).add(rank));
  };
    ...
    <div className="glass-panel-accent flex flex-col">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-1">
              <button
                ...
                className={`px-3 py-1 rounded text-[10px] font-semibold tracking-wider transition-all ${
                  tab === "community" ? "bg-neon-cyan/15 text-neon-cyan border border-neon-cyan/20" : "text-muted-foreground hover:text-foreground"
                }`}
              >
                🗳️ COMMUNITY
              </button>
              <button
                onClick={() => setTab("real")}
                className={`px-3 py-1 rounded text-[10px] font-semibold tracking-wider transition-all ${
                  tab === "real" ? "bg-neon-cyan/15 text-neon-cyan border border-neon-cyan/20" : "text-muted-foreground hover:text-foreground"
                }`}
              >
                📊 DATA
              </button>
              ...
              {["All", "ICBM", "HGV", "Cruise"].map((t, i) => (
                <button
                  ...
                  className={`px-2.5 py-1 rounded text-[10px] font-semibold tracking-wider transition-all ${
                    i === 0 ? "bg-neon-green/10 text-neon-green border border-neon-green/20" : "text-muted-foreground hover:text-foreground"
                  }`}
                >
                  {t}
                </button>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin px-2 py-1">
        {topWeapons.map((w, i) => (
          <div
            key={w.rank}
            className={`animate-row-in grid grid-cols-[36px_1fr_70px_70px_70px_60px_60px_36px] gap-2 items-center px-3 py-2.5 rounded-md mb-1 border transition-all duration-200 hover:bg-muted/30 group cursor-pointer ${rankGlow(w.rank)}`}
            style={{ animationDelay: `${i * 80}ms` }}
          >
            {/* Rank */}
            <span className={`text-lg font-bold ${rankColor(w.rank)}`}>
            ...
            <div className="flex items-center gap-1">
              <span className="text-sm">{w.countryCode}</span>
              <span className="text-[10px] text-muted-foreground hidden xl:inline">{w.country}</span>
            </div>

            {/* Votes or Range */}
            {tab === "community" ? (
              <span className="text-xs font-mono-tech text-neon-cyan">{voteCounts[w.rank]?.toLocaleString()}</span>
            ) : (
            ...
            {/* Vote button */}
            <button
              onClick={(e) => { e.stopPropagation(); handleVote(w.rank); }}
              className={`flex items-center gap-0.5 px-1.5 py-1 rounded text-[10px] font-bold transition-all ${
                votedItems.has(w.rank)

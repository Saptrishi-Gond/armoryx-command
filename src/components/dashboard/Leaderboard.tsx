...
const Leaderboard = () => {
  ...
  const categories = ["All", "ICBM", "HGV", "Cruise", "SLBM", "Hypersonic", "Aircraft"];

  const filtered = topWeapons.filter(w =>
    ...
    <div className="glass-panel-accent flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin px-2 py-1">
        {sorted.map((w, i) => (
          ...
          >
            <span className={`text-lg font-bold ${rankColor(w.rank)}`}>{String(w.rank).padStart(2, "0")}</span>
            <div>
